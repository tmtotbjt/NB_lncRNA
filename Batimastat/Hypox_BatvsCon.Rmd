...
```{r}
g <- readRDS( "~/NB_lncRNA/output/code/BioInsights/graph/network_graph_HBC_padj.RDS")
V(g)$name <- V(g)$symbol
tg <- readRDS("~/NB_lncRNA/output/code/BioInsights/graph/tbl_HBC_padj.rds")
```
## lncRNA ir mRNA tinklas
Turima 16 mazgus ir 18 ryšius tarp ilgųjų nekoduojančių RNR ir baltymų arba baltymo-baltymo
```{r}
table(V(g)$type)
vcount(g)
...
ecount(g)
```
Vienas didesnis ir kitas mažesnis (pagal komponentų viename tinkle sk.)
```{r}
components(g)$csize
```
...
```{r}
lnc_enrichment_results <- readRDS("~/NB_lncRNA/output/code/BioInsights/graph/lncEn_HBC_padj.RDS")
```
...
```{r}
lncGR <- readRDS("~/NB_lncRNA/output/code/BioInsights/Bastimat/lncGR_HBC_padj.RDS")
geneGR <- readRDS("~/NB_lncRNA/output/code/BioInsights/Bastimat/geneGR_HBC_padj.RDS")
