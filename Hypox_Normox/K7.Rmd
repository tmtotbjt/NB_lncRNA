...
sampleInfo <- tmp[[2]]
samples2do <- c("K7cH1", "K7cH2", "K7cN1", "K7cN2")
...
genesFC <- rownames(res)[res$logFC >= log2(3) & res$FDR < 0.05]
...
lnc_top <- lnc_res[abs(lnc_res$logFC) > 1.25 & lnc_res$FDR < 0.05, ]
lnc_mat_top <- lnc_mat[rownames(lnc_mat) %in% rownames(lnc_top), ]

pc_res <- res[rownames(res) %in% rownames(pc_mat), ]
pc_top <- pc_res[abs(pc_res$logFC) > 2 & pc_res$FDR < 0.05, ]
...
Koreliuojantys genai susiję su: reakcija į kintančiį deguonies kiekį, skeleto raumeninio audinio augimu, inkstais ir antinksčiais (kortikolis, vamzdelis)
```{r}
