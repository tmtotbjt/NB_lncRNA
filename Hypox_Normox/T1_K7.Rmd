...
```{r}
g <- readRDS( "~/NB_lncRNA/output/code/BioInsights/graph/network_graph_TK_padj.RDS")
V(g)$name <- V(g)$symbol
tg <- readRDS("~/NB_lncRNA/output/code/BioInsights/graph/tbl_TK_padj.rds")
```
## lncRNA charakteristikos
Turima 332 mazgus ir 585 ryšius tarp ilgųjų nekoduojančių RNR ir baltymų arba baltymo-baltymo
```{r}
table(V(g)$type)
vcount(g)
