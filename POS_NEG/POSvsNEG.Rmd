...
## Koreliacija pagal mėginius PCA
(iš visų)
```{r}
...
geneGR <- readRDS("~/NB_lncRNA/output/code/BioInsights/POS_NEG/geneGR.RDS") #sig pakitę pc RNA (log 2)
...
entrez_ids_all <- bitr(pc_names, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)
...
# Persiskaiciuoti atskirai (nes vienas is klonu durniuoja)
```
...
### Atskirai sudarius koreliacijas
```{r}
# Idėja:
## Atskirai filtruojamės kiekvieną tipą atsikrai
## Žiūrim ar yra bednros koreliacijos (ta pati lncRNA su ta pačia mRNA)
## Esant bendrom koreliacijom MANO MANYMU eliminuojasi matomas skirtumas tarp klonų

## Bet ar tos koreliacijos nesiamto bendroje? Ar bendroje matome dar kažką? Kaip koreliacijja tokiu atveju veikia?
## Aj ne, jau kitaip bus
## Aš gi atfiltravau jau klonų viduje, jau kitokios reikšmės gaunasi
...
cor_sig_T1 <- subset(cor_df_T1, abs(value) > 0.85)
cor_sig_K7 <- subset(cor_df_K7, abs(value) > 0.85)
