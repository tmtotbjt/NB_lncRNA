...
top_lnc <- head(top_lnc, n=15)
ids <- V(g)[name %in% top_lnc$name]

neighbors <- unique(unlist(ego(g, order = 1, nodes = ids)))

subgraph <- induced_subgraph(g, neighbors) %>%
  as_tbl_graph()

protein_threshold <- quantile(
    subgraph %>% 
    activate(nodes) %>% 
    as_tibble() %>% 
    filter(type == "protein") %>% 
...
subgraph <- subgraph %>%
  activate(nodes) %>%
  filter(type == "lncRNA" | degree >= protein_threshold)
...
```{r}
lncGR <- readRDS("~/NB_lncRNA/output/code/BioInsights/POS_NEG/lncGR_PN_padj.RDS")
...
sig_lncRNAs$rn <- rownames(sig_lncRNAs)
res_lnc <- sig_lncRNAs %>%
